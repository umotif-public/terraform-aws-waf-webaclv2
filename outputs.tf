...
output "web_acl_assoc_id" {
  ...
  value       = var.create_alb_association && var.enabled && var.alb_arn != "" ? join("", aws_wafv2_web_acl_association.main[0].*.id) : ""
}

output "web_acl_assoc_resource_arn" {
  description = "The ARN of the ALB attached to the Web ACL Association"
  value       = var.create_alb_association && var.enabled && var.alb_arn != "" ? join("", aws_wafv2_web_acl_association.main[0].*.resource_arn) : ""
}

output "web_acl_assoc_acl_arn" {
  description = "The ARN of the Web ACL attached to the Web ACL Association"
  value       = var.create_alb_association && var.enabled && var.alb_arn != "" ? join("", aws_wafv2_web_acl_association.main[0].*.web_acl_arn) : ""
}
