...
| <a name="input_custom_response_body"></a> [custom\_response\_body](#input\_custom\_response\_body) | A custom response body to be referenced on a per rule basis. | `map(string)` | `{}` | no |
| <a name="input_description"></a> [description](#input\_description) | A friendly description of the WebACL | `string` | `null` | no |
| <a name="input_enable_custom_response"></a> [enable\_custom\_response](#input\_enable\_custom\_response) | Set to `true` to define custom responses (status code, body, and/or headers) for non managed rule group statements block actions. | `bool` | `false` | no |
